...
### 2. Little Endian (强制小端序)

**规则**: 所有多字节整数**必须**使用小端序 (Little Endian)。

```python
# Python
struct.pack('<HH', x, y)   # ✅ 正确 (< 表示小端)
struct.pack('HH', x, y)    # ❌ 错误 (字节序不确定)

# Java/Kotlin
ByteBuffer buffer = ByteBuffer.allocate(4);
buffer.order(ByteOrder.LITTLE_ENDIAN);  // ✅ 必须设置!
buffer.putShort(x);
```
...
**维护说明**: 本协议遵循 **Binary First** 原则。添加新功能时，严禁使用 JSON 传输核心数据，必须使用紧凑的二进制结构。所有多字节整数必须使用小端序 (Little Endian)。
